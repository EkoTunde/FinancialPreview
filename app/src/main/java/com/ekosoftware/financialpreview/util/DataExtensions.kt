...
fun Frequency.forDisplay(): String {
    Log.d(TAG, "forDisplay: from>${this.from} to>${this.to} installments>${this.installments}")
    return when {
        this.from == this.to -> "${this.from!!.monthNameKey()}-${this.from!!.getYear()}"
        to == 999999 -> Strings.get(
            R.string.repeats_placeholder_from,
            this.from!!.monthNameKey(),
            this.from!!.getYear(),
        ...
        this.installments != null || this.installments!! > 0 -> Strings.get(R.string.installments_frequency_placeholder, this.installments!!)
        else -> Strings.get(
            R.string.repeats_placeholder_to,
            this.from!!.monthNameKey(),
            this.from!!.getYear(),
            this.to!!.monthNameKey(),
            this.to!!.getYear(),
            if (this.monthsChecked == Strings.get(R.string.all_months_checked_frequency)) {
                Strings.get(R.string.every_month)
            } else Strings.get(R.string.some_months)
