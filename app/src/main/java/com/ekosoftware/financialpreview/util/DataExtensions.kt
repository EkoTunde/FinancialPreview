...
/**
 * Removes zeros (if any) from a [String] representing a number with fraction units in decimal points.
 */
...
/**
 * Returns a new [Date] for given days ago from now.
 *
 * @param days the amount of days before today.
 */
fun getDateDaysAgo(days: Int): Date {
    val today = LocalDateTime()
    val dateDaysAgo = today.minusDays(days)
    return dateDaysAgo.toDate()
}

/**
 * Returns a new random-generated id in [String] format.
 */
fun newId() = UUID.randomUUID().toString()

/**
 * Creates a copy of *this* [Movement] with new random-generated id in [String] format.
 */
fun Movement.duplicate(): Movement = this.apply { id = newId() }

/**
 * Creates a copy of *this* [Budget] with new random-generated id in [String] format.
 */
fun Budget.duplicate(): Budget = this.apply { id = newId() }
