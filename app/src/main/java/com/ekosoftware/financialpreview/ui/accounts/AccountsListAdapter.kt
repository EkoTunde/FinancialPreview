...
class AccountsListAdapter(private val onSelected: (v: View, Account) -> Unit) :
    ...
    override fun bind(item: Account, binding: ItemAccountBinding) = with(binding) {
        item.colorResId?.let { colorContainer.setBackgroundColor(Colors.get(it)) }
        name.text = item.name
        /*amountTxt.text =
        ...
        amountTxt.applyMoneyFormat(item.currencyCode, item.balance.forDisplayAmount(item.currencyCode).toDouble())
    }
